/**
 * Routes proxy pour les notifications (Edge Function Supabase)
 */
 ...
 * Envoie une notification push FCM via l'Edge Function send-user-notification
 */
router.post('/send', verifySupabaseToken, async (req, res) => {
  try {
    const response = await fetch(`${SUPABASE_URL}/functions/v1/send-user-notification`, {
      method: 'POST',
      headers: {
    ...
    console.error('[notifications/send] Erreur:', err.message)
